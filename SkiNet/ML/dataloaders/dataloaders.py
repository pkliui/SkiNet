...
class _RepeatSampler(BatchSampler):
    ...
    In combination with a modified Pytorch's Dataloader, this sampler does not result in starting new worker processes
    at the beggining of a new epoch. This is because the workers are shut down when the sampler is exhausted:
    see the _MultiProcessingDataLoaderIter._next_data method.
...
class RepeatDataLoader(DataLoader):
    ...
    It is a subclass of PyTorch's DataLoader and uses a custom BatchSampler to repeat the dataset
    indefinitely or a given number of times.
    ...
    def __init__(self,
        ...
        :param max_num_to_repeat: The maximum number of times the dataset should be repeated.
            If set to 0, the dataset will be repeated indefinitely.
        :param batch_size: The number of samples per minibatch. Default is 1.
        :param shuffle: If True, the dataset will be shuffled randomly using RandomSampler, otherwise SequentialSampler will be used.
            Default is False.
        ...
        repeat_sampler = _RepeatSampler(
            self.repeatdl_batch_sampler, batch_size=batch_size, max_num_to_repeat=max_num_to_repeat)
        logging.getLogger(__name__).debug(
            f"BatchSampler repeat_sampler length {len(repeat_sampler)}")
        logging.getLogger(__name__).debug(
            f"BatchSampler repeat_sampler {repeat_sampler}")
