import pytest
from SkiNet.ML.utils.sampling.encoder_sampling import PaddingMode, get_padding_stride_2, get_padding_and_stride, handle_mixed_inputs
...
class TestGetPaddingAndStride:
    ...
    def test_get_padding_and_stride_integration_with_handle_mixed_inputs_success(self, kernel, dilation, expected_padding, padding_mode):
        ...
        Integration with handle_mixed_inputs for success cases
        """
        ps = get_padding_and_stride(kernel, dilation, padding_mode)
        assert ps.padding == 'same' if padding_mode == PaddingMode.SAME else (expected_padding, expected_padding) # we have num_dims=2 by default
        assert ps.stride == 1 if padding_mode == PaddingMode.SAME else 2
